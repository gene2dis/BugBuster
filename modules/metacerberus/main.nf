process METACERBERUS_READS {
    ...
    publishDir "${params.output}/workflow/${meta.id}/metacerberus_reads", pattern: '*_annotation'   
    publishDir "${params.output}/Annotation/${meta.id}/metacerberus_reads", mode: 'copy', pattern: '*annotation_results'

    input:
        tuple val(meta), path(reads)

    output:
        tuple val(meta), path("*_annotation"), emit: all_files
        path("*annotation_results"), emit: results
    ...
    script:
        ...
        """
        cp -L ${reads[0]} tmp_read_1.fastq.gz 
        cp -L ${reads[1]} tmp_read_2.fastq.gz

        gunzip tmp_read_1.fastq.gz
        gunzip tmp_read_2.fastq.gz
