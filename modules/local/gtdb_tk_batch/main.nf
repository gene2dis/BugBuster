...
process GTDB_TK_BATCH {
    ...
    for meta_id in ${meta_ids}; do
        # Find bins for this sample (metawrap bins are in input_bins/<sample_id>/)
        if [ -d "input_bins/\${meta_id}" ]; then
            bin_count=\$(find -L input_bins/\${meta_id} -maxdepth 1 -type f \\( -name "*.fa" -o -name "*.fasta" -o -name "*.fna" \\) 2>/dev/null | wc -l)
            
            if [ "\$bin_count" -gt 0 ]; then
                echo "Found \$bin_count bins for sample \${meta_id}"
                # Copy bins with prefixed names to track origin
                for bin_file in input_bins/\${meta_id}/*.fa input_bins/\${meta_id}/*.fasta input_bins/\${meta_id}/*.fna; do
                    if [ -f "\$bin_file" ]; then
                        bin_name=\$(basename "\$bin_file")
                        cp "\$bin_file" "combined_bins/\${meta_id}__\${bin_name}"
