...
\u001B[33mOptional arguments:\u001B[0m
   ...
   \u001B[32m--arg_bin_clustering\u001B[0m               ARG gene prediction and clustering for horizontal gene transfer inference 
                                  (default: false)
...
//         ch_autometa = AUTOMETA(ch_bowtie2_samtools.contigs_and_bam, params.ncbi_db)

         ch_all_bins = ch_metabat2.join(ch_semibin).join(ch_comebin)
         ch_metawrap = METAWRAP(ch_all_bins, params.metawrap_db)

        // ASIGNACION TAXONOMICA DE CONTIGS, BLOBPLOT Y PREDICCION DE ARGS 

         if ( params.contig_tax_and_arg) {

             ch_nt_blastn = NT_BLASTN(ch_filtered_contigs.bbmap, params.nt_db)
             ch_index_bam = SAMTOOLS_INDEX(ch_bowtie2_samtools.only_bam)
             ...
             ch_contig_proteins = PRODIGAL_CONTIGS(ch_filtered_contigs.bbmap_contigs)
             ch_contig_args = DEEPARG_CONTIGS(ch_contig_proteins, params.deeparg_db)
             ARG_CONTIG_LEVEL_REPORT(ch_contig_args.only_deeparg.concat(ch_blob_table.only_blob).collect())
        }
    ...
    if ( params.arg_bin_clustering) {
        ...
        ch_raw_orfs.join(ch_deeparg).view()
        
//        ch_clusters = CLUSTERING(ch_deeparg)
    }
