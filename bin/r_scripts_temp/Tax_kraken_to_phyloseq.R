...
#importar archivos, cambiar nombres y preparar tablas y metadata para objeto phyloseq
assign("File_names",
       list.files(path = '.',pattern = "*.biom"))

data_bases <- c()
for (name in File_names) {
  data_bases <- c(data_bases, unlist(strsplit(name,"_"))[1])
  
  df <- paste0(data_bases,"_merged")
  df_tax_table_data <- paste0(data_bases,"_merged_tax_table_data")
  df_otu_table_data <- paste0(data_bases,"_merged_otu_table_data")
  df_phyloseq <- paste0(data_bases,"_phyloseq")
  df_phyloseq_fix <- paste0(data_bases,"_phyloseq_fix")
  ...
  if(!is.null(test)){
    assign(df, import_biom(name))
  } else {
    assign(df, new_import_biom(name))
  }
  
  assign(df_tax_table_data, get(df)@tax_table@.Data %>%
           substring(4))
  
  assign(df_tax_table_data, `colnames<-`(get(df_tax_table_data),c("Kingdom", "Phylum", "Class",
                                           "Order", "Family", "Genus", "Species")))
  
  assign(df_otu_table_data, get(df)@otu_table@.Data)
  assign("colnames", colnames(get(df_otu_table_data)))
  assign("fix_colnames", gsub(paste0("_",data_bases), "", get("colnames")))
  assign(df_otu_table_data, `colnames<-`(get(df_otu_table_data),get("fix_colnames")))
  
  assign(df_phyloseq, phyloseq(otu_table(get(df_otu_table_data),
                                         taxa_are_rows=TRUE), 
                               tax_table(get(df_tax_table_data))))
  
  saveRDS(get(df_phyloseq), file = paste0("kraken2_",data_bases,"_phyloseq.RDS"))
}
